...
import { getAssistantCourse } from "@/lib/assistant";
import { saveQuiz, saveQuestion, deleteQuestion, uploadQuizImage, getCourseStudentPerformance } from "@/lib/admin-quizzes";
import { 
  Plus, 
  Trash2, 
  Edit2, 
...
function AssistantExamsContent() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // 1. Verify Assistant owns this course
      const course = await getAssistantCourse(courseId);
      ...
      if (quizzes && quizzes.length > 0) {
        ...
        // Auto create default quiz
        const defaultTitle = lessonId 
          ? `كويز محاضرة: ${lessonTitleText || "الدرس"}` 
      ...
      console.error("فشل جلب تفاصيل التقييم:", error.message);
      alert("تعذر فتح التقييم: " + error.message);
  ...
  async function handleImportCourseChange(cId: string) {
    setSelectedImportCourseId(cId);
    ...
    try {
      ...
      alert("فشل تحميل امتحانات الكورس: " + err.message);
    }
  }

  async function handleImportQuizChange(qId: string) {
    setSelectedImportQuizId(qId);
    setSelectedQuestionIds([]);
    if (!qId) {
      setImportQuestions([]);
      return;
    ...
    try {
      ...
      alert("فشل تحميل أسئلة الامتحان: " + err.message);
    }
  }
  ...
  async function handleDeleteQ(qId: string) {
    if (!confirm("هل أنتِ متأكدة من حذف هذا السؤال نهائياً؟")) return;
    try {
  ...
  return (
    <div className="space-y-8" dir="rtl">
      ...
      <div>
        <Link 
          href={lessonId ? `/assistant/lesson?courseId=${courseId}` : `/assistant/courses`}
        ...
        <h1 className="text-2xl md:text-3xl font-extrabold text-[#2D2B7A]">
          {lessonId ? "📚 إدارة كويز المحاضرة" : "🏆 إدارة الامتحان الشامل النهائي"}
        </h1>
        ...
        <div className="bg-white p-6 rounded-3xl border shadow-sm h-fit space-y-6">
          ...
          <form onSubmit={handleSaveQuiz} className="space-y-4">
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              />
              <label htmlFor="is_active_ast" className="text-xs font-bold text-[#2D2B7A] cursor-pointer">
                الامتحان مفعل ومتاح للطلاب (نشر الامتحان)
              </label>
            </div>
            ...
            <div className="space-y-4">
              ...
              {questions.length === 0 ? (
                <div className="bg-white rounded-3xl p-12 border text-center text-gray-400 font-medium">
                  لا توجد أسئلة مضافة في هذا الامتحان حالياً. اضغطي "إضافة سؤال جديد" أو "استيراد أسئلة" للبدء.
                </div>
              ) : (
                <div className="space-y-4">
                  {questions.map((q, idx) => (
                    <div key={q.id} className="bg-white rounded-3xl p-6 border shadow-sm space-y-4">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-hidden border border-gray-100 animate-in fade-in zoom-in-95 duration-200 flex flex-col" dir="rtl">
            ...
            <form onSubmit={handleSaveQuestion} className="p-6 overflow-y-auto space-y-4 flex-1">
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5">صيغة السؤال (نص)</label>
                <textarea
                  rows={3}
              ...
              {/* Question Image */}
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5">صورة السؤال (اختياري للمسائل والرسومات)</label>
                <input
                  type="file"
                  accept="image/*"
              ...
              <div className="space-y-3 border-t pt-4">
                <label className="block text-xs font-extrabold text-[#2D2B7A]">خيارات الإجابة الأربعة</label>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-hidden border border-gray-100 animate-in fade-in zoom-in-95 duration-200 flex flex-col" dir="rtl">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">اختر الكورس المصدر</label>
                <select
              ...
              {selectedImportCourseId && (
                <div>
                  <label className="block text-xs font-bold text-gray-500 mb-1">اختر التقييم (امتحان أو كويز)</label>
                  <select
                    className="w-full px-4 py-2.5 rounded-xl border outline-none text-[#2D2B7A] font-bold text-sm bg-white cursor-pointer"
                    value={selectedImportQuizId}
                    onChange={(e) => handleImportQuizChange(e.target.value)}
                  >
                    <option value="">-- اختر الامتحان / الكويز --</option>
                    {importQuizzes.map((q) => (
                      <option key={q.id} value={q.id}>
                        {q.title} ({q.type === "quiz" ? "كويز محاضرة" : "امتحان شامل"})
                      </option>
                    ))}
                  </select>
                </div>
              ...
              {selectedImportQuizId && (
                <div className="space-y-3 pt-2">
                  <div className="flex justify-between items-center text-xs font-bold text-gray-500">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-[#7D79F1] hover:underline cursor-pointer"
                      >
                        تحديد الكل
                      </button>
                      <span>|</span>
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto border p-3 rounded-xl bg-gray-50/50">
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            className="w-4 h-4 text-[#7D79F1] rounded cursor-pointer shrink-0 mt-0.5"
                          />
                          <div>
...
export default function AssistantExamsPage() {
  return (
    <Suspense fallback={<div className="text-center py-12 text-gray-500 font-bold">جاري تحميل صفحة التقييم...</div>}>
      <AssistantExamsContent />
    </Suspense>
  );
}
